...
clisp eqStr.lisp
sbcl --script eqStr.lisp
ccl64 -l eqStr.lisp -e '(quit)'
...
(cond
  ( ; condition START
    (equal str1 str2)
    (format t "~%~a~a~a~a~%~%" cGreen "Equal" cNormal " (case-sensitive).")
    #+clisp
      (quit 0)
    #+sbcl
      (quit :unix-status 0)
  ...
  ( ; default condition START
    t
    (format t "~%~a~a~a~a~%~%" cRed "Not equal" cNormal " (case-sensitive).")
    #+clisp
      (quit 1)
    #+sbcl
      (quit :unix-status 1)
